class OperacionMatrices():
    ...
    def transformada29Caracol(self, dimension, numInicio):
        M = [[0] * dimension for i in range(dimension)]
        for c in range(0,int(dimension/2)):
            for ls in range(c,dimension-1-c):
                M[c][ls] = numInicio
    ...
    def transformada27(self, dimension, numInicio):
        M = [[0] * dimension for i in range(dimension)]
        contX=dimension-1
        while(contX>=0):
            if (contX % 2 == 0):
                ...
                contY = 0
            while(contY>=0 and contY<=dimension-1):
                M[contX][contY] = numInicio
                numInicio = numInicio + 1
                if contX % 2 == 0:
                        contY = contY-1
                else:
...
obj.imprimirMatriz(obj.transformada27(dimension,numInicio))